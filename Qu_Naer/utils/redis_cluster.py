...
import redis
from redis._compat import b, iteritems, iterkeys, itervalues, basestring, bytes
...
class StrictRedisCluster:
    ...
    _write_keys = {
        ...
        'expire': 'expire', 'expireat': 'expireat', 'pexpire': 'pexpire', 'pexpireat': 'pexpireat', 'getset': 'getset',
        'hdel': 'hdel',
        ...
        'zrem': 'zrem', 'zremrangebyrank': 'zremrangebyrank', 'zremrangebyscore': 'zremrangebyscore',
    ...
    _loop_keys = {
        ...
        'bgrewriteaof': 'bgrewriteaof', 'info': 'info',
            ...
            if name not in StrictRedisCluster._loop_keys:
                ...
                if name in StrictRedisCluster._tag_keys and not tag_start:
                    try:
                        ...
                        raise redis.DataError(
                            "rediscluster: Command %s Not Supported (each key name has its own node)" % name)
                ...
                for alias, redisent in iteritems(self.redises):
                    if (name in StrictRedisCluster._write_keys and alias.find('_slave') >= 0) or (
                                name in StrictRedisCluster._read_keys and alias.find('_slave') == -1):
                        res = None
                    else:
